...
ZQ <- load.data(as.matrix(read.csv("FR-Q.csv", header=FALSE)))
ZM <- load.data(as.matrix(read.csv("FR-M.csv", header=FALSE)))
ZD <- load.data(as.matrix(read.csv("FR-D.csv", header=FALSE)))
ZG <- load.data(as.matrix(read.csv("FR-Google.csv", header=FALSE)))
...
for(jh in 1:NROW(hh))
{
  ...
  h <- hh[jh]
  # give name
  give.name <- paste("FR-Q-itarg1", itarg, "-h", h, sep="")
