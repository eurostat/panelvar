...
wd1 <- dirname(sys.frame(1)$ofile) # absolute path to current model directory
...
ZQ <- load.data(as.matrix(read.csv("UK-Q.csv", header=FALSE)))
ZM <- load.data(as.matrix(read.csv("UK-M.csv", header=FALSE)))
ZD <- load.data(as.matrix(read.csv("UK-D.csv", header=FALSE)))
ZG <- load.data(as.matrix(read.csv("UK-Google.csv", header=FALSE)))
...
ds <- max(c(min(ZQ$dates), min(ZM$dates), min(ZD$dates),
          min(ZG$dates)))
de <- min(c(max(ZQ$dates), max(ZM$dates), max(ZD$dates),
            max(ZG$dates)))

xselQ <- (ZQ$dates>=ds) & (ZQ$dates<=de)
...
for(jh in 1:NROW(hh))
{
  ...
  h <- hh[jh]
  # give name
  give.name <- paste("UK-Q-itarg1", itarg, "-h", h, sep="")
  
  # make sure you start the loop on the same date
  iinfrom <- "2009-09-30"
  ifrom <- which(rownames(XQ)==iinfrom)
  ...
  fest <- matrix(NA, NROW(XQ), NROW(mnams))
  rownames(fest) <- rownames(XQ)
  colnames(fest) <- mnams
  ...
  for(i in ifrom:(NROW(XQ)-h))
  {
    # in-sample
    xq <- XQ[1:i, ]
    
    # Using the above xq, construct the corresponding monthly datasets
    dq <- as.Date(rownames(xq))
    dm <- crspn.dates.QtoM(dq)
    xm <- XM[as.character(dm),]
    
    # Finally, make daily/weekly to monthly using averaging
    xd <- make.data.DWtoM(dm, as.matrix(XD))
    ...
    Fm <- na.omit(cbind(xms, xds)); rnams <- rownames(Fm)
    Fm <- getPCA(Fm); rownames(Fm) <- rnams
    Fg <- na.omit(xgs); rnams <- rownames(Fg)
    Fg <- getPCA(Fg);  rownames(Fg) <- rnams
    
    # Extract target
    y <- as.matrix(xqs[,ZQ$target==itarg])
    ...
    xx <- make.data.MtoQ1(as.Date(rownames(y)), as.matrix(cbind(x1, x2)))
    colnames(xx) <- paste("X", 1:NCOL(xx), sep="")
    
    # Transform monthly factors to quarterly
    # according to the quarterly dates of the target
    xF1 <- make.data.MtoQ1(as.Date(rownames(y)), as.matrix(Fm[,1]))
    xG1 <- make.data.MtoQ1(as.Date(rownames(y)), as.matrix(Fg[,1]))
    xFGR1 <- cbind(xF1, xG1)
    
    xF2 <- make.data.MtoQ2(as.Date(rownames(y)), as.matrix(Fm[,1]))
    xG2 <- make.data.MtoQ2(as.Date(rownames(y)), as.matrix(Fg[,1]))
    xFGR2 <- cbind(xF2, xG2)
    ...
    cat("Now doing ", i, " of ", (NROW(XQ)-h), " and h ", h, "\n")
  }
  
  fwhich <- which(ZQ$target==itarg)
  ftrue <- as.matrix(XQ[,fwhich])
  ftrue <- transform.vars(ftrue, ZQ$transf[fwhich])
